...
async function detail(id) {
    ...
    const vod = {
        ...
        vod_content: '[关注公众号:影视资源站]  ' + $('div.mod-box-5 > article > p').text(),
    };
	const playlists = $('div.numList > ul > li > a');
	vod.vod_play_from = '公众号|影视资源站';
	const playurl = [];
...
async function play(flag, id, flags) {
	...
	let palyUrl = "";
	for(const id of nmjx){
		try{
			console.debug(id);
			html = await request(id + encUrl);
...
function substringBetween(id, open, close) {
  const start = id.indexOf(open);
  if (start !== -1) {
    const end = id.indexOf(close, start + open.length);
    if (end !== -1) {
      return id.substring(start + open.length, end);
    }
  }
  return '';
}
...
async function getNmJx(){
	let result = await request("https://igdux.top/~nmjx");
	nmjx = JSON.parse(result)['data'];
	//console.debug(nmjx);
	//return jx;
}
